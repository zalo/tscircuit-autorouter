...
export interface HgPortPointPathingSolver2Params {
...
export class HgPortPointPathingSolver2 extends HyperGraphSolver<
  TypedRegion,
  TypedRegionPort
> {
  ...
  constructor(private params: HgPortPointPathingSolver2Params) {
    super({
  ...
  override computeRoutesToRip(
    ...
    newlySolvedRoute.path.map((candidate) => {
      assertDefined(
        ...
        "Candidate in solved route has undefined last region",
      )
      assertDefined(
        candidate.lastPort,
        "Candidate in solved route has undefined last port",
      )
    ...
    const traversedRegions = newlySolvedRoute.path.map((candidate) => {
      assertDefined(
        candidate.lastRegion,
        "Candidate in solved route has undefined last region",
      )
